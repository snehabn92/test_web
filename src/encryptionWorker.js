...
self.onmessage = async (event) => {
    const { action, key, readable, writable, shouldSendEncryptionKey } = event.data;
    switch (action) {
        case "generateKey":
            await generateKey();
...
async function setKey(key) {
    encryptionKey = await crypto.subtle.importKey("raw", key, { name: "AES-GCM" }, true, ["encrypt", "decrypt"]);
